...
module.exports = {
	...
	execute: async (bot, msg, args, cfg) => {
		...
		if (!clear) {
			...
			catch(e) { return "We are unable to access that URL. Please try another."; }
			...
			if(Number(head.headers["content-length"]) > 1048575) {
				return "That image is too big and Discord so here's what i found. Please use an image under 1mb.";
			}
			let res;
			try { res = await probe(url); }
			catch(e) { return "There was an error checking that image. Please try another."; }
